...
contract DEX {
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        ...
        // Chuyển token từ người tạo sàn vào contract
    ...
    function price(uint256 xInput, uint256 xReserves, uint256 yReserves) public pure returns (uint256 yOutput) {
        return ((xInput * 997) * yReserves) / (xReserves * 1000 + xInput * 997);
    }
    ...
    function ethToToken() public payable returns (uint256) {
        ...
        uint256 yOutput = price(xInput, xReserves, yReserves);

        token.transfer(msg.sender, yOutput);
    ...
    function deposit() public payable returns (uint256 tokenDeposited) {
        ...
        tokenDeposited = (msg.value * tokenReserve) / ethReserve + 1;
        ...
        bool sent = token.transferFrom(msg.sender, address(this), tokenDeposited);

        require(sent, "deposit: transferFrom failed");

        emit LiquidityProvided(msg.sender, liquidityMinted, msg.value, tokenDeposited);
    ...
    function withdraw(uint256 amount) public returns (uint256 ethAmount, uint256 tokenAmount) {
        ...
        require(ethSent, "Eth withdrawn failed");

        bool tokenSent = token.transfer(msg.sender, tokenAmount);
        require(tokenSent, "Token withdrawn failed");
